...
export async function verifyAdminAccess(request) {
  // TODO: Implement your actual authentication/authorization logic
  // Example:
  // const session = await getServerSession(authOptions);
  // if (!session?.user?.isAdmin) return null;
  // return { userId: session.user.id, isAdmin: true };
  
  return { userId: "mock-user-id", isAdmin: true };
}
...
export function parsePagination(searchParams, maxLimit = 100) {
  ...
  const limit = Math.min(parseInt(searchParams.get("limit") || "10"), maxLimit);
...
export function validateDateRange(startDate, endDate, fieldName = "date") {
  ...
  if (end <= start) {
    return errorResponse(`End ${fieldName} must be after start ${fieldName}`, 400);
  }
